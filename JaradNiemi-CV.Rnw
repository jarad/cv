...
in_prep = pubs %>% filter(status=="in preparation")
...
ur = pubs %>% filter(status=="under revision") %>% arrange(desc(sort_order))
...
sub = pubs %>% filter(status=="submitted") %>% arrange(desc(sort_order))
...
tmp = pubs %>%
  filter(status=="published", since_promotion) %>%
  arrange(-year) %>%
  rowwise() %>%
  do(published_print(.))

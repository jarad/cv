...
Data Science Program & Iowa State University \\
\quad Director & 2024--present \\
\\
Statistics & Iowa State University \\
\quad Professor & 2023--present \\
\quad Associate Professor & 2017--2023 \\
\quad Assistant Professor & 2011--2017 \\
\\
Statistics and Actuarial Sciences & Simon Fraser University \\
\quad Visiting Scholar & 2019-2020 \\
\\
Statistics \& Applied Probability & University of California,\ Santa Barbara \\
\quad Assistant Professor & 2009--2011
\end{tabular}
...
Consultant 
 & Eocene Environmental Group, Inc. & 2024--2026 \\
 & Kemin Industries & 2024 \\
 & Boehringer Ingelheim Vetmedica, Inc. & 2012--2013 \\
...
<<read_pubs, echo=FALSE, cache=TRUE, cache.extra = c(tools::md5sum('publications.csv'), tools::md5sum('people.csv'), tools::md5sum('aliases.csv'), tools::md5sum('studentcommittees.csv')), message=FALSE>>=
...
people     = readr::read_csv("people.csv")
aliases    = readr::read_csv("aliases.csv")
committees = readr::read_csv("studentcommittees.csv")
...
decorate_authors = function(s) {
  ...
  for (i in seq_len(nrow(spellings))) {
    ...
    if (pid == "jarad-niemi")        out = gsub(esc, paste0("{\\bf ",esc,"}"), out, fixed=TRUE)
    else if (pid %in% advisee_ids)   out = gsub(esc, paste0(esc,"*"),          out, fixed=TRUE)
  }
  out
}
...
pubs = readr::read_csv("publications.csv") %>%
  arrange(-year) %>%
...
\subsection{\bf At other universities} \vspace{-0.2in}

``Bayesian methodology for disease severity forecasting,''
University of Nebraska, Medical Center, 10 Oct 2025

``Bayesian methodology for disease severity forecasting,'' 
Medical College of Wisconsin, 2 May 2025


\sinceappt

``Building statistical emulators of computer models,'' St. Olaf College, 14 Nov 2022

``Toward emulators for agricultural computer models,'' Virginia Tech, 3 Nov 2022

``COVID-19 forecast hub,'' University of Iowa, 15 Oct 2020

``Sequential knot selection in sparse Gaussian processes,''
Simon Fraser University, 10 Jan 2020

``Fully Bayesian analysis for hierarchical count regression models.'' University of California, Irvine, 28 Nov 2017

``Fully Bayesian analysis of RNAseq data for gene expression heterosis detection.'' University of Massachusetts - Amherst, 24 March 2017
...
\subsection{\bf At conferences} \vspace{-0.2in}

``Bayesian success in disease severity forecasting.'' 
Best of Statistical Science (BOSS) 2026, 24 Apr 2026

``Specifying prior distributions in reliability applications.''
SIAM Conference on Uncertainty Quantification (UQ26), 23 Mar 2026

\sinceappt

``Gaussian processes with functional length-scales for agricultural models.''
Statistical Society of Canada Annual Meeting, 11 Jun 2021

``Multivariate temporal modeling of crime with dynamic linear models.''
Joint Statistical Meetings, 6 Aug 2020

``OAT knot selection for approximate Gaussian processes.''
SIAM Conference on Uncertainty Quantification  UQ20, 
March 2020 (canceled due to COVID-19)

``Parallelized Markov chain Monte Carlo algorithms utilizing GPUs with an application to RNAseq data analysis.'' 9th International Conference of the ERCIM WG on Computational and Methodological Statistics, University of Seville, Spain, 9-11 December 2016
...
``Particle learning for sequential estimation and prediction of disease outbreaks.''  2013 ICSA-ISBS Joint Statistical Conference, 10 Jun 2013

``Particle learning for low counts in disease outbreaks.'' Midwest Statistics Research Colloquium, 15 Mar 2013

``Rejection sampling on a graphical processing unit.'' 11th World Meeting of the International Society for Bayesian Analysis, 28 Jun 2012
...
\subsection{\bf At agencies, companies, other venues} \vspace{-0.2in}

\sinceappt

``Emulation of Agricultural Production Systems sIMulator (APSIM).''
Los Alamos National Labs, 23 Sep 2020

``Hierarchical modeling and data fusion.'' Centers for Disease Control and Prevention Seasonal Influenza Forecasting Workshop, 31 Aug 2016

``Stochastic dynamic models for low count observations.'' Institute for Disease Modeling Symposium, 19 Apr 2016

``Particle learning for low counts in disease outbreaks.'' Institute for Disease Modeling, 18 Apr 2016





\section{\bf Contributed Talks at Major Conferences}

{\small

\sinceappt

``Subfield Yield Analysis for Precision Agriculture.'' Joint Statistical Meetings, 30 July 2019

``Fully Bayesian Analysis of Hierarchical Count Regression Models.''
Joint Statistical Meetings, 31 July 2018
...
\section{\bf Other Talks}

{\small

\sinceappt

``Gaussian Process Emulators with Functional Inputs for Water Erosion Prediction Project'' NCCC170, 15 Jun 2023

``Advancing Scientific Practice through Agricultural Statistics,'' Iowa State University, 26 Sep 2022

``Computer Model Emulation in Agriculture.'' NCCC170, 16 Jun 2022

``COVID-19 Forecast Hub.'' ISU Undergraduate STAT Club, 22 Oct 2020

``Bayesian analysis in R.'' Rgronomists/LunchinatoRs, 1 Feb 2019

``Emulation of WEPP and DEP.'' SAMSI MUMS Prediction Uncertainty and Extrapolation working group, 28 Nov 2018

``Using information underlying missing data for improve estimation of NFL field goal kicker accuracy.'' Midwest Sports Analytics Meetings, 19 Nov 2016
...
``Empirical Bayes analysis for detection of gene heterosis in RNAseq data.'' Conference on Applied Statistics in Agriculture, Manhattan, KS, 27 Apr 2015
...
``Statistical methods for identifying gene expression heterosis.'' Conference on Applied Statistics in Agriculture, Manhattan, KS, 30 Apr 2013 (joint with Dan Nettleton)
...
``Time management.'' UCSB IGERT Career Development Seminar Series, 12 Apr 2011
...
``A brief introduction to R.'' University of California, Santa Barbara Quantitative Methods in Social Sciences seminar, 8 Apr 2010
...
``Discrete-time models for intracellular processes in systems biology.'' Graduate Student Research Day, Duke University, 02 Apr 2008
...
{\bf Jarad Niemi} and Will Landau.
Fully Bayesian analysis of hierarchical count regression models applied to RNAseq.
2018 ISBA World Meeting, 26 Jun 2018

{\bf Jarad Niemi} and Will Landau.
{\tt fbseq}: An R package for fully Bayesian analysis of RNAseq data.
...
{\bf Jarad Niemi} and Mike West. ``Bayesian parameter estimation for systems biological models of dynamic cellular networks.'' 8\textsuperscript{th}Valencia International Meeting on Bayesian Statistics, 5 Jun 2006
...
\hline
Spring 2026 & DS 4010 & Data Science Capstone \\
Fall 2025 & STAT 6150 & Advanced Bayesian Methods \\
Spring 2025 & STAT 4610X & Sports Analytics \\
& DS 4010 & Data Science Capstone \\
Fall 2024 & STAT 5870-1 & Statistical Methods For Research Workers \\
Spring 2024 & STAT 486/586 & Introduction to Statistical Computing \\
& STAT 544 & Bayesian Statistics \\
Spring 2023 & STAT 486/586 & Introduction to Statistical Computing \\
Fall 2022 & STAT 330 & Probability and Statistics for Computer Science \\
Spring 2022 & HCI 522 & Scientific Methods in Human Computer Interaction \\
Fall 2021 & STAT 587-1 & Statistical Methods For Research Workers \\
& STAT 615 & Advanced Bayesian Methods \\
Spring 2021 & STAT 330-2 & Probability and Statistics for Computer Science \\
& STAT 587-3 & Statistical Methods For Research Workers \\
Fall 2020 & STAT 587-2 & Statistical Methods For Research Workers \\
Spring 2019 & STAT 544 & Bayesian Statistics \\
& STAT 587C & Statistical Methods for Research Workers \\
Fall 2018 & STAT 226 & Introduction to Business Statistics \\
Spring 2018 & STAT 544 & Bayesian Statistics \\
 & STAT 401C & Statistical Methods for Research Workers \\
Fall 2017 & STAT 615 & Advanced Bayesian Methods \\
Spring 2017 & STAT 401D & Statistical Methods for Research Workers \\
 & STAT 544 & Bayesian Statistics \\
Fall 2016 & STAT 330 & Probability and Statistics for Computer Science \\
...
\begin{tabular}{llll}
Jan 2022 & Data Management Workshop & ISU & 1-day \\
Dec 2016 & Introduction to R for Biosurveillance & ISDS & 1-day \\
...
\begin{itemize}
\sinceappt
\item Iowa State University Presidential Interdisciplinary Research Initiative (ISU-PIRI), HYDRA: Hydration and Defense Resource Advancement,
Role: Investigator, 
1 July 2025 to 30 June 2028
\item USDA NIFA (\#2020-68012-31824)
%Lisa Schulte Moore, Thomas Richard, Rudi Roselein, J. Gordon Arbuckle, Robert Brown, Alexis Campbell, Jaquiline Comito, Emily Heaton, Matthew Helmers, Kevin Kimle, Matt Liebman, Mark Mba Wright, {\bf Jarad Niemi}, Dvid Swenson, Michael Thompson, John Tyndall, Andy VanLoocke, Wen Zhiyou, Saurabh, Daniel Ciolkosz, Christine Costllo, Lara Fowler, Mark Gagnon, Kathleen Hill, C. Clare Hinrichs, Alexander Hristov, Matthew Johnson, Heather Karsten, Armen Kemanian, Hilal Ezgi Toraman, Thomas Schwartz, and Steven Trabue,
Consortium for Cultivating Human and Naturally reGenerative Enterprises (C-CHANGE),
Role: Co-PI.
1 Sep 2020 to 31 Aug 2025.
[\$10,000,000; \$62,614 to Niemi]
\item Foundation for Food and Agriculture Research,
%Richard Cruse, Matt Helmers, Matt Liebman, Marshall McDaniel, Bradley Miller, {\bf Jarad Niemi}, Lisa Schulte, and John Tyndall,
Prairie Strips for Healthy Soils and Thriving Farms,
Role: Co-PI
1 October 2018 to 30 September 2021
[\$746,204.44; \$69,747 to Niemi]
% (allocation to Niemi: $\sim$ 6\%)
\item Iowa State University Presidential Interdisciplinary Research Initiative (ISU-PIRI),
% Lisa Schulte-Moore, J. Gordon Arbuckle, Robert Brown, Emily Heaton, Matt Helmers, Adam Janke, Kevin Kimle, Matt Liebman, Marshall McDaniel, {\bf Jarad Niemi}, and Dara Wald.
Initiative for Cultivating Human And Naturally reGenerative Enterprises (I-CHANGE),
Role: Co-PI.
1 June 2018 to 31 May 2021
[\$741,480; \$6,175 to Niemi]
% (allocation to Niemi: $\sim$ 12\%)

\item D. Nettleton, P. Liu, {\bf J. Niemi}, P. Schnable. Hierarchical Modeling and Parallelized Bayesian Inference for the Analysis of RNAseq Data, National Institutes of Health, September 1, 2013 to May 31, 2017 [\$1,088,156; \$272,039 to Niemi]
\end{itemize}
...
\begin{itemize}
\item Iowa State University Emerging Leaders Academy (2025--2026)
\item 2025 Iowa State University College of Liberal Arts \& Sciences Inclusive Excellence (Team Award including Ulrike Genschel, Laura Ziegler, and Ann Hawkins)
\sinceappt
\item \href{https://archive.las.iastate.edu/2022/08/10/celebrate-faculty-and-staff-at-las-fall-convocation-and-awards-ceremony/}{2022 Iowa State University College of Liberal Arts \& Sciences Outstanding Achievement in Extension or Professional Practice}
\item \href{https://www.inside.iastate.edu/article/2022/04/28/awards}{2022 Iowa State University Interdisciplinary Team Research Award} (\href{https://cchange.research.iastate.edu/}{C-CHANGE})
\item 2021 American Society of Agronomy \href{https://acsess.onlinelibrary.wiley.com/doi/full/10.1002/agj2.21069}{Presidential Award} (\href{https://nccc170.netlify.app/}{NCCC170})
\item \href{https://www.swcs.org/resources/news/soil-and-water-conservation-society-recognizes-international-leaders-in-conservation}{2020 Conservation Innovation Award}, Soil and Water Conservation Society (\href{http://prairiestrips.org/}{STRIPS Team})
\item \href{https://news.las.iastate.edu/2018/05/14/las-announces-2018-award-winners/}{2018 Iowa State University College of Liberal Arts \& Sciences Institutional Service Award}
\item \href{https://www.cals.iastate.edu/features/2018/science-based-trials-rowcrops-integrated-prairie-strips-project-received-team-award}{2018 Iowa State University College of Agriculture and Life Sciences Team Award} (\href{http://prairiestrips.org/}{STRIPS Team})
...
\item NVIDIA Academic Partnership Program award [2$\times$ NVIDIA C2050 GPUs] (2010)
...
{\bf Profession}
\begin{itemize}
\item American Statistical Association Iowa Chapter Treasurer (2026--present)
\item NSF Panel (2018, 2024, 2025)
\item Associate Editor for \emph{Journal of Agricultural, Biological, and Environmental Statistics} (2016--2026)
% \item Refereeing
%  \begin{itemize}
%  \item
...
%  SIAM Undergraduate Research Online (2017),
...
\item Conference organization
 \begin{itemize}
 \item Conference on Predictive Inference in Sports, Conference Chair (2025)
 \item Conference on Applied Statistics in Agriculture and Natural Resources, Program Committee Member (2024)
...
\item Other statistical community service (hyperlinked if appropriate)
 \begin{itemize}
 \item First Robotics Competition (FRC) \href{https://www.teamneutrino.org/}{Team Neutrino (\#3928)} Mentor (2024--current)
 \item Iowa State Science \& Technology Fair Judge (2022--2023)
 \item First Lego League (FLL) Coach (2022--2024)
 \item \href{https://nccc170.netlify.app/}{NCCC170 Website} Development Lead (2022--present)
 \item Cross-validated user \href{http://stats.stackexchange.com/users/40440/jaradniemi}{4,561 rep}
 ...
 \item News Editor for Significance Magazine (2010--2012)
 % \item Professional twitter account ({\tt \href{https://twitter.com/jaradniemi}{@jaradniemi}})
 \item Professional YouTube channel (\url{https://www.youtube.com/jaradniemi}) (2011--present)
%\item Boy Scouts of America troop presentation on estimating player abilities in basketball
 \end{itemize}

\item {\bf University service}
  \begin{itemize}
  \item ISU Provost Working Group on Faculty Salary Equity Study (2022--2024)
  \item ISU LAS CASTLE (2020--2025, chair 2023--2025)
  \item C-CHANGE Smart Technologies Lead (2020--2024)
  \item ISU STRIPS Data lead (2018--2024)
  \item ISU Faculty Compensation Committee (2016--2022)
  \item ISU Transportation Advisory Council (2015--2018)
  \item ISU Faculty Senate departmental representative (2012--2018)
...
\item {\bf Departmental/Program service}
 	\begin{itemize}
 	\item ISU Data Science Program
 	  \begin{itemize}
 	  \item Administrative Assistant Search Committee, co-chair joint with Psychology (2026)
    \item Data Science Program Director (2024--current)
    \item Data Science Curriculum Committee (chair 2024--current)
 	  \end{itemize}
	\item ISU Department of Statistics
		\begin{itemize}	
		\item Communications Specialist Search Committee, member (2025)
		\item Statistical Consulting Director (2025--current)
		\item Graduate Recruiting and Admissions Committee, member (2020--2026), director (2022--2025)
		\item Data Science Club, advisor (2025--current)
		\item Sports Analytics Club, advisor (2020--current)
		\item Graduate Student Services Specialist Search Committee, member (2022, 2023)
		\item Faculty Search Committee, member (2018--2019, 2021--2022, 2025--2026), chair (2023--2024)
		\item Graduate Curriculum and Exams Review, co-chair (2022--2024)
		\item Graduate Curriculum Review Committee, chair (2021--2022)
		\item Graduate Exams Review Committee, member (2016--2019, 2021--2023)
		\item Diversity, Equity, and Inclusion Committee, member (2020--2021)
		\item Website Committee, member (2020--current), chair (2020--2024)
		\item Computation Advisory Committee, member (2011--2022, 2023--current), chair (2017--2019)
		\item Department Chair Search Advisory committee (2012--2014, 2018--2019)
		% \item Data Science Faculty search committee (2018--2019)
    \item Snedecor Sustainability Committee, co-chair (2015--2017)
  	\item Preparing Future Faculty mentor:
  	  \begin{itemize}
  	  \item Will Landau (2013--2014)
  	  \item Natalia da Silva (2015--2016)
  	  \end{itemize}
  	\item STAT-ers advisor (2011--2017)
...
<<read_studentcommittees, cache.extra = c(tools::md5sum("studentcommittees.csv"), tools::md5sum("people.csv"))>>=
# library(plyr)
library(dplyr)
library(xtable)
...
d = readr::read_csv("studentcommittees.csv")
...
<<undergraduate-research, results='asis', cache.extra = tools::md5sum("undergraduate_research.csv")>>=
d3 = read.csv("undergraduate_research.csv") %>%
